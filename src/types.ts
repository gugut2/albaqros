...
export interface ProjectArtifact {
  ...
  autoGeneratedCover?: boolean; // True if cover was automatically extracted from Krita/Photoshop/Blender
  coverSource?: string; // 'krita' | 'photoshop' | 'blender' | 'shell'
}
...
// ==========================================
// Canvas Studio Types (Obsidian Canvas format)
// ==========================================

export type CanvasNodeType = 'text' | 'image' | 'note' | 'group';
...
export interface CanvasNode {
  ...
  text?: string;
  // For image node:
  src?: string; // base64, file:// or url
  alt?: string;
  aspectRatio?: number;
  fitMode?: 'contain' | 'cover';
  // For note node:
  notePath?: string; // relativePath to markdown note
  noteTitle?: string;
  notePreview?: string;
  // For group node:
  label?: string;
...
export interface CanvasMetadata {
  id: string; // relative path, e.g. "GameDesign/character-map.canvas"
  title: string; // e.g. "character-map"
  fileName: string; // e.g. "character-map.canvas"
  relativePath: string; // e.g. "GameDesign/character-map.canvas"
  folder: string; // e.g. "GameDesign" or ""
  createdAt: string;
  updatedAt: string;
  nodeCount?: number;
}

export interface CanvasDocument extends CanvasMetadata {
  data: CanvasData;
}
